...
def multinomial_loss_function(x_logit, x, z_mu, z_var, z, beta=1.):
    ...
    :param z_0: attribute_seperation stochastic latent variable
...
def synthesize_feature_match_model(netG,RN, dataset, opt):
    gen_feat = torch.FloatTensor(dataset.ntest_class * opt.nSample, opt.X_dim+opt.C_dim)
    ...
    with torch.no_grad():
        for i in range(dataset.ntest_class):
            ...
            _, _, G_sample_B, _ = RN(G_sample)
            gen_feat[i * opt.nSample:(i + 1) * opt.nSample] = G_sample_B
...
def save_model_ae(it, model,ae, random_seed, log, fout):
    torch.save({
        'it': it + 1,
