...
class DATA_LOADER(object):
    ...
    def read(self, opt):
        ...
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/data.mat")     # 直接读取data
        ...
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/label.mat")  #读取lable
...
class FeatDataLayer(object):
    ...
    def _shuffle_roidb_inds(self):   # 进行对训练的特征的图片的顺序进行重新排列
        """Randomly permute the training roidb."""
        # 在这里使用np.arange()来生成一个序列
        # 接着调用np.random.permutation()将序列进行随机排列
        # _perm得到的是一个7057的随机序列
